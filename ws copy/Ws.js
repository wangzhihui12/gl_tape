...
export class Ws {
  ...
  subscribe() {
    this.ws.on(WsTyps.Connected, () => {
      uni.$logger.local.info('========WebSocket 连接成功========')
    })
    this.ws.on(
      WsTyps.MessageTypes.Private,
      function (data) {
        uni.$logger.local.info(`WebSocket 收到消息 -> ${JSON.stringify(data)}`)
        if (data.errorCode && data.errorCode == 907) return this.connect()
    ...
    this.ws.on(WsTyps.Disconnected, () => {
      ...
      uni.$logger.local.info(`========WebSocket 断开连接 device_id->${this.deviceId}========`)
    })
