  ...
  methods: {
    ...
    async getALlCourse() {
      try {
        ...
        const params = {
          learnStatus: 0, // 学习状态 0：待学习 1: 正在学习 2：已学完
          pageNum,
          pageSize
        }
        ...
        // 临时添加测试数据字段
        const listWithTestData = res.list.map((item, index) => ({
          ...item,
          required: index % 3 === 0, // 每3个课程第1个是必修
          courseType: index % 2 === 0 ? 'video' : 'document', // 交替显示视频/文档
          rating: (Math.random() * 2 + 3).toFixed(1), // 随机评分 3.0-5.0
          studentCount: Math.floor(Math.random() * 1000) + 100 // 随机学习人数 100-1100
        }))

        if (pageNum == 1) {
          this.recommendCourseList = listWithTestData
        } else {
          this.recommendCourseList = this.recommendCourseList.concat(listWithTestData)
        }
        uni.hideLoading()
    ...
    startStudy(item, index) {
      ...
      uni.navigateTo({
        url: `/pages/common/studyCenter/courseDetail?apiType=getMyCourseList&index=${index}&courseList=${encodeURIComponent(JSON.stringify(this.recommendCourseList))}&pageNum=${pageNum}&pageSize=${pageSize}&total=${total}&learnStatus=0`
      })
    },
  }
}
