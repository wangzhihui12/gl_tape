<template>
  <view class="detail-box">
    <view class="nav-bar">
      ...
      <view class="title">{{ type === 'recommendation' ? '推荐课程' : '任务详情' }}</view>
    </view>
        ...
        <view class="scroll-view-box">
          <scroll-view class="scroll-view" scroll-y  @scrolltolower="pageonReachBottom">
            <view :class="['scroll-view-item', selectedIndex == index ? 'active' : '']"
              v-for="(item, index) in getCourseList" :key="index" @click="openCourse(index, item)">
              <template v-if="type === 'long'">
                <image class="item-img" src="../../../assets/images/common/long-course-default.webp"
                  mode="scaleToFill" />
                <view class="item-content">
                  ...
                  <view class="item-info">
                    ...
                    <view :class="['item-info-value', item.status === 5 ? 'green' : 'orange']">
                      <view v-if="item.status !== 5" class="dot-value"></view>
                      {{ item.status !== 5 ? `${statusMap[item.status]}（${item.progress === 0 ? 0 : item.progress || 0}%）` : '已完成' }}
                    </view>
                  </view>
                  <view class="item-info">
                    <view class="item-info-label">考试成绩</view>
                    <!-- <view class="item-info-value">{{ (item.totalScore || item.totalScore === 0) ? item.totalScore : '待考核' }}</view> -->
                ...
                <image class="completed-bg" src="../../../assets/images/common/completed-bg.webp" mode="scaleToFill"
                  v-if="item.status === 5" />
                <view v-else class="item-btn" @click="openCourse(index, item)">
                  开始学习
                </view>
              ...
              <template v-if="type === 'recommendation'">
                <image class="recommendation-img" src="../../../assets/images/common/recommend-course-default2.webp"
                  mode="scaleToFill" />
                <view class="item-content">
                  <view class="recommendation-title">{{ item.courseName }}</view>
                  <view class="item-info">
                    <view class="item-info-label">{{ (item.totalDuration || item.totalDuration === 0) ? `${item.totalDuration}min` : '-' }}</view>
                  </view>
                </view>
                <view class="item-btn" @click="openCourse(index, item)">
                  学习
                </view>
              </template>
            </view>
...
export default {
  ...
  onLoad(options) {
    ...
    if (options.recommendCourseList) {
      this.recommendCourseList = JSON.parse(decodeURIComponent(options.recommendCourseList));
      courseId = this.recommendCourseList[options.index].courseId;
    }
    console.log(this.recommendCourseList, 'this.recommendCourseList')
