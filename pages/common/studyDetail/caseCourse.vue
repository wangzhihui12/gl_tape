    ...
    <view class="course-list-box-wrapper">
      ...
      <scroll-view class="course-list-box" scroll-y>
        <view class="course-list-item-wrapper">
          <view class="course-list-item" @click="startStudy(index, item)" v-for="(item, index) in rowData.courseList"
            :key="index">
  ...
  methods: {
    ...
    startStudy(index, item) {
      uni.$api.commonApi.findBeiSenSSOToken({}).then(res => {
        ...
        if (beisenToken) {
          uni.$track.add({ eventCode: 'app_long_course', businessId: item.id, pagePath: '/pages/common/studyDetail/caseCourse' })
          uni.navigateTo({
            url: `/pages/common/studyDetail/detail?type=long&index=${index}&courseList=${encodeURIComponent(JSON.stringify(this.rowData.courseList))}`
          })
        } else {
          uni.showToast({
            title: '培训账号未创建',
