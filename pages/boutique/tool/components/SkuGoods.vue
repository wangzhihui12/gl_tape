  ...
  <view>
    <view class="shop-list">
      <view class="shop-item" v-for="(item, index) in list" :key="index">
        ...
        <view class="right">
          <view class="shop-name">商品名称：{{ item.multipleSpecificationsGoods ? item.multipleSpecificationsGoods.baseGoodsName : item.goods.goodsAlias || item.goods.upgradeGoodsName || item.goods.goodsInfo.goodsName }}</view>
          ...
          <view class="front-gear-box">
            <view class="front-gear-input-box">
              ...
              <template v-if="item.goods">
                <template v-if="(item.goods.merchantGoodsPosition || item.goods.upgradeGoodsPosition) && (item.goods.goodsInfo.goodsType == 1 || item.goods.upgradeGoodsType == 1 || item.goods.goodsInfo.goodsType == baseConfigData.selfBuildFilmType || item.goods.upgradeGoodsType == baseConfigData.selfBuildFilmType)">
                  <view class="front-gear-label">
                    {{ formatPosition(item.goods.merchantGoodsPosition || item.goods.upgradeGoodsPosition, positionTypeList) }}
                  </view>
                </template>
                <template v-if="(item.goods.goodsInfo.goodsType == 1 || item.goods.upgradeGoodsType == 1) && !item.goods.merchantGoodsPosition && !item.goods.upgradeGoodsPosition">无部位属性</template>
              </template>
              <view class="front-gear-input">
                <view class="unit">¥</view>
                <template v-if="item.multipleSpecificationsGoods">
                  <input class="uni-input" type="digit" maxlength="9" :disabled="isDisabled" placeholder="请输入价格" placeholder-style="font-size: 14px" v-model="item.multipleSpecificationsGoods.goods.sellPrice" @blur="validatePrice($event, item, index)" />
                </template>
                <template v-else>
                  <input class="uni-input" type="digit" :disabled="isDisabled" maxlength="9" placeholder="请输入价格" placeholder-style="font-size: 14px" v-model="item.goods.sellPrice" @blur="validatePrice($event, item, index)" />
                </template>
                <!-- <input class="uni-input" type="digit" maxlength="9" placeholder="请输入价格"
                  placeholder-style="font-size: 14px" 
                  :value="item.multipleSpecificationsGoods && item.multipleSpecificationsGoods.goods ? item.multipleSpecificationsGoods.goods.sellPrice : item.goods.sellPrice"
                  @blur="validatePrice($event, item, index)" /> -->
              </view>
            </view>
            <view class="control">
              <view class="minus" @click="decreaseNumber(item)">-</view>
              <template v-if="item.multipleSpecificationsGoods">
                <input class="uni-input control-input" type="number" maxlength="3" v-model="item.multipleSpecificationsGoods.number" @blur="validateNumber($event, item)" />
              </template>
              <template v-else>
                <input class="uni-input control-input" type="number" maxlength="3" v-model="item.goods.number" @blur="validateNumber($event, item)" />
              </template>
              <!-- <input class="uni-input control-input" type="number" maxlength="3"
                :value="item.multipleSpecificationsGoods && item.multipleSpecificationsGoods.goods ? item.multipleSpecificationsGoods.goods.number : item.goods.number"
                @blur="validateNumber($event, item)" /> -->
...
export default {
  ...
  methods: {
      ...
      if (this.list && this.list.length > 0) {
        this.list.forEach(item => {
          if (!item.multipleSpecificationsGoods) {
            ...
            // 只有当 number 字段完全不存在（undefined/null）时才设置默认值，避免覆盖回显的数量
            if (item.goods && (item.goods.number === undefined || item.goods.number === null)) {
              this.$set(item.goods, 'number', '1')
            }
          } else if (item.multipleSpecificationsGoods) {
            // 如果有多规格商品，给multipleSpecificationsGoods中的goods添加number
            // 只有当 number 字段完全不存在（undefined/null）时才设置默认值，避免覆盖回显的数量
            if (item.multipleSpecificationsGoods.number === undefined || item.multipleSpecificationsGoods.number === null) {
    ...
    validatePrice(event, item, index) {
      ...
      if (!item.multipleSpecificationsGoods) {
        ...
        if (item.multipleSpecificationsGoods.goods.validMinSellPrice === 'Y' && item.multipleSpecificationsGoods.goods.minSellPrice && parseFloat(value) < parseFloat(item.multipleSpecificationsGoods.goods.minSellPrice)) {
          console.log('最小值', item.multipleSpecificationsGoods.goods.minSellPrice)
          uni.showToast({
            title: '单价不可低于商品最低价',
            icon: 'none'
          })
    ...
    resetStrategyWhenPriceChange(item) {
      ...
      let number = 1
      if (item.multipleSpecificationsGoods && item.multipleSpecificationsGoods.number) {
    ...
    recalculateFinalPrice(item) {
      ...
      if (goods && goods.strategyConfig && goods.finalPricePerUnit !== undefined) {
