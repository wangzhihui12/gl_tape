...
export default {
  ...
  data () {
    return {
          // 订单状态 （0待报价、1待确认、2待支付、3已支付、5已完成、6已退款、9已撤销、11错误订单、-1战败）
      list: Object.freeze([
        {
          name: '待报价',
          value: 0,
  ...
  methods: {
    initList () {
      ...
      console.log(data, idKey ,'32938928392')
      if (data[idKey] !== undefined && data[idKey] !== null && data[idKey] !== '') {
        let item = list.find(val => val.value == data[idKey])
