<template>
  <view class="employee-list">
    <view
      @click="currentIds = []"
      :class="['item flex',{active:!currentIds.length}]"
    >
    ...
    async getList () {
      ...
      const { shopMerchantId, sceneType, merchantId } = userInfo,
        data = await uni.$api.popupApi.getOrderScreenEmployeeList({
          merchantId: params.idKey == 'sellerUserIdList' ? merchantId : shopMerchantId,
          sceneList: params.idKey == 'sellerUserIdList' ? '' : [sceneType]
        })
      if (data && data.length) {
        this.initList(data)
