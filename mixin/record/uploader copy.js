  ...
  methods: {
    ...
    async checkDB() {
      try {
        ...
        uni.$logger.error(`【上传管理Uploader】 == 初始化数据库失败:${error.message}`)
        this.toLogin()
      }
    },
    /**
    ...
    async save(data) {
      try {
        ...
        uni.$logger.local.info(`【上传管理Uploader】 == 解析结果 -> ${JSON.stringify(fileInfo)}`)
        if (!fileInfo || !fileInfo.recordId) {
          uni.$logger.local.error(`【上传管理Uploader】 == 上传文件信息解析异常`)
          return
        }
     ...
     */
    parseFileInfo(data) {
      uni.$logger.local.info(`【上传管理Uploader】 == 开始文件信息解析-> ${JSON.stringify(data)}`)
      // 如果是无文件状态，直接返回基础信息
      if ([FileStatus.NoFile, FileStatus.NotExist].includes(data.status)) {
        let { recordId, filePath, fileName, status } = data
        status = status == FileStatus.NotExist ? FileStatus.Completed : FileStatus.NoFile //Completed NoFile 状态用于区分 正常上传的Uploaded状态.后续获取服务器文件路径过滤
        return { recordId, clientPath: filePath + fileName, serverPath: null, fileName, status }
      }
      // 获取上传结果，如果失败直接返回 null
      const { recordId, fileName, filePath, code, data: fileData, message } = data.detail.data[0]
      if (code !== 0) {
        const __message = `【上传管理Uploader】 == 文件上传回调,解析异常: code->${code} message->${message}`
        uni.$logger.local.error(__message)
        this.reportError(__message, data, recordId, filePath + fileName)
        return null
      }
      // 返回成功上传的文件信息
    ...
    _changeFilePathToServer(record, fileInfo) {
      try {
        ...
        uni.$logger.local.info(`【上传管理Uploader】 == 开始将接待记录的文件路径改成服务器路径!! record.recordUrlList-> ${JSON.stringify(_record.recordUrlList)}`)
        // 如果是无文件状态，直接返回基础信息
        if (fileInfo.status === FileStatus.NoFile) {
          ...
          uni.$logger.local.info(`【上传管理Uploader】 == 当前记录没有接待语音文件,已将状态设置为无文件状态: ${JSON.stringify(_record)}`)
          return _record
        }
        // console.log(_record)
        // 正常文件上传和文件丢失的记录执行此逻辑
        _record.recordUrlList = this.urlListFilter(_record.recordUrlList)
        const item = _record.recordUrlList.find(item => {
          const isLegal = this.isLegalFile(fileInfo.fileName)
          return isLegal ? item.file === fileInfo.clientPath || item.file.endsWith(fileInfo.fileName) : !this.isLegalFile(item.file)
        })
        if (item) {
          let info2Update = {
          ...
          uni.$logger.local.info(`【上传管理Uploader】 == 修改文件路径完成!!`)
        } else {
          uni.$logger.local.info(`【上传管理Uploader】 == 未找到记录 ==  fileInfo -> ${JSON.stringify(fileInfo)}`)
        }
        uni.$logger.local.info(`【上传管理Uploader】 == 替换完成!! record.recordUrlList-> ${JSON.stringify(_record.recordUrlList)}`)
        return _record
      } catch (error) {
        uni.$logger.error(`【上传管理Uploader】 == 修改文件路径失败!! error-> ${error.message}`)
    ...
    async enQueue() {
      try {
        ...
        uni.$logger.local.info('【上传管理Uploader】 == 创建快照成功! 文件个数:' + this.snapshot.size)
      } catch (error) {
    ...
    async uploadRecord(record) {
      ...
      if (record.status === FileStatus.NoFile || record.status === FileStatus.NotExist) {
        ...
        try {
          ...
          uni.$logger.local.info(`【上传管理Uploader】 == 获取Token成功!!', lsaccessKeySecret: ${lsaccessKeySecret}`)
    ...
    _removeFileOnClient(filePathOnClient) {
      uni.$logger.local.info(`【上传管理Uploader】 == 开始删除客户端的文件!! 路径-> ${filePathOnClient}`)
    ...
    async _saveOrUpdate(data) {
      ...
      try {
        ...
        if (status === FileStatus.NoFile || fileList.every(i => [FileStatus.Uploaded, FileStatus.Completed].includes(i.status))) {
          ...
          uni.$logger.local.info(`【上传管理Uploader】 == 已上传的文件: ${JSON.stringify(pathArr)}`)
          uni.$logger.local.info(`【上传管理Uploader】 == 本地文件: ${JSON.stringify(originalFileArr)}`)
          ...
          uni.$logger.local.info(`【上传管理Uploader】 == 服务器返回结果: ${JSON.stringify(res)}`)
          // 如果服务器返回结果成功
          if (res) {
            // 移除本地文件
    ...
    async polling() {
      try {
        ...
        if (this.snapshot === null) return uni.$logger.info('stooooooooooooop')
        if (this.$store.getters.isInBackgound) {
          uni.$logger.info('【上传管理Uploader】 == APP在后台运行,不进行文件上传!')
          this.waitting()
          return
        }
        // 检查网络是否在线
        ...
        if (isOnline) {
          ...
          this.reportError('【上传管理Uploader】 == 网络异常,稍后再试')
          this.waitting()
        }
      } catch (error) {
    ...
    async beginTask(record) {
      ...
      this.operationQueue.enqueue(() => this.task(record), `文件ID: ${record.recordId}@${record.fileName}`).finally(this.polling)
    },
