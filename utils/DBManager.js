 ...
 * @LastEditors: limin
 * @LastEditTime: 2025-08-06 00:56:24
 * @FilePath: \gl_tape\utils\DBManager.js
 */
...
class DBManager {
  ...
  table_columns = [`id TEXT PRIMARY KEY`, `staffUuid TEXT`, `staffName TEXT`, `serviceMerchantId INTEGER`, 'carModel TEXT', 'endReasonType INTEGER', 'customerName TEXT', 'phoneNumber TEXT', 'appCode TEXT', 'loginPhone TEXT', 'salesName TEXT', 'salesUuid TEXT', 'receptionStatus INTEGER', 'receptionStartTime TEXT', 'receptionEndTime TEXT', 'receptionCustomerPaySourceList TEXT', 'merchantId INTEGER', 'merchantName TEXT', 'bussinessType INTEGER', 'channelType INTEGER', 'receptionCustomerMaterialPointList TEXT', 'recordUrlList TEXT', 'carBrand TEXT', 'carBrandId INTEGER', 'oneCarBrand TEXT', 'oneCarBrandId INTEGER']
  sqlite = null
  isTableExists = false
  constructor(table_name, table_columns) {
    this.table_name = table_name || this.db_table_name
    this.table_columns = table_columns || this.table_columns
  ...
  async init() {
    try {
      ...
      uni.$logger.info(`数据库是否存在isTableExists: ${isTableExists}`)
      if (!isTableExists) {
